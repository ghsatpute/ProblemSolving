...
for i in range(0, numCases) :
    ...
    mat = [0 for j in range(rows)]
    for i in range(0, rows) :
        mat[i] = raw_input()
    
    #print mat
    
    # Input matrix to search
    row_s, col_s = map(int, raw_input().split(' '))
    search = [0 for j in range(row_s)]
    ...
    found = False
    # Search the matrix
    for i in range(0, rows - row_s + 1) :
        for j in range(0, cols - col_s + 1) :
            # Try to match the first number
            for k in range(0, row_s) :
                if(not mat[i + k][j:].startswith(search[0 + k])) :
                    #print mat[i + k][j:], "\t", search[0 + k], "FALSE"
                    break
                #else :
                    #print mat[i + k][j:], "\t", search[0 + k], "TRUE"
                if k == row_s - 1 :
                    found = True
