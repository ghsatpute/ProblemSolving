...
for i in range (0,T):
    ...
    numChocolates = int(A / B)
    answer += numChocolates
    wrappers = numChocolates
    
    while(wrappers >= C) :
        # Buy the chocolate
        numChocolates = int(wrappers/C)
        wrappers -= numChocolates * C
        
        answer += numChocolates
        # Adding wrappers of chocolates just brought
        wrappers += numChocolates
