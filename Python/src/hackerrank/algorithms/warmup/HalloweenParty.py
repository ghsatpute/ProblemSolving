...
If the chocolate can be served as only 1 x 1 sized pieces and Alex can cut the chocolate bar exactly  times, what is the
...
Output Format
  lines; each line should contain an integer that denotes the maximum number of pieces that can be obtained for each
  test case.
...
Note: Chocolate must be served in 1 x 1 sized pieces. Alex can't relocate any of the pieces, nor can he place any piece
on top of another.
...
for i in range(0, numTestCases):
    ...
    print (int(numCuts / 2)) * (numCuts - int(numCuts / 2))
